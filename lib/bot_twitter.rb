...
#****************************************************************************
	...
	def bot_twitter(tbl)
		...
		if client_stat != nil
			if 1 == 0 #To avoid 
				5.times do 
					client_stat.update("Bonjour #{(tbl.shuffle)[0]}, je suis en formation chez THP @the_hacking_pro et je dois tweeter aléatoirement... c'est tombé sur toi ... désolé.. <3 #bonjour_monde")
				end
				client_stat.filter(track: "#bonjour_monde") do |tweet|
				  puts tweet.text
				end
				client_stat.search("#bonjour_monde").take(25).each do |tweet|
					client_stat.follow(tweet.user.screen_name)
				end
				client_stat.search("#bonjour_monde").take(20).each do |tweet|
